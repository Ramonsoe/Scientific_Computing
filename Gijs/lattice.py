...
class Lattice:
    ...
    def check_if_in_object(self):
        if (self.objects[self.walker[0], self.walker[1]] == 1):
            # This means that the random walker moved into the object
            # Then we should return a False:
            return True
        else:
            return False
            
    def MC_lattice_probability(self, eta):
        """
        This method is similar to MC_lattice: the random walker picks a 
        direction to move in and checks if it can move there.
            Difference: if the object is found, there is a probability of sticking
        to the object, instead of always increasing the object size.
        """
        ...
        # Small difference in beginning: if the random walker did not stick
        # to the object in last iteration, it could be the case that 
        # the walker moved into the object now, which should not be possible:
        if (self.check_if_in_object() == True):
            self.walker[direction] = self.walker[direction] - up_or_down
            # so then, the walker is simply moved back, and we start over
            ...
            # If we are next to the object, the random walker should be added to it
            if (self.check_neighbours(i,j) == True):
                # Now, there should also be a probability of sticking:
                p = np.random.rand()
                if (p < eta):
                    # Random uniform draw smaller than eta: so stick to object.
                    self.objects[i,j] = 1
                    self.lattice[i,j] = 0
                    self.initialize_walker()
